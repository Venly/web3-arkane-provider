...
function isAddress(address: string): boolean {
    if (!BASIC_ADDRESS_REGEX.test(address)) return false;
    else if (SAME_CASE_ADDRESS_REGEX.test(address)) return true;
    else {
...
export abstract class BaseWalletSubprovider extends Subprovider {
    ...
    private async _emitSendTransactionAsync(signedTx: string): Promise<any> {
        const payload = {
